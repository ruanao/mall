    ...
    <section class="newGoods section">
      ...
      >
        <ul class="goodsList" :style="{width:`${(266*goodsList.length)+(10*(goodsList.length-1))}px`}" slot="list">
          <!-- <li class="GoodsItem" v-for="(item,index) in goodsList" :key="item.id" :style="{marginRight: (index+1)%4===0?'0px':'10px'}">
            <div class="imgBox">
              <img :src="item.img" alt="商品图片" @click="navTo('/mall/goods/'+item.id)"/>
            </div>
            <div class="goodsInfo">
              <span class="goodsName ellipsis" @click="navTo('/mall/goods/'+item.id)">{{item.name}}</span>
